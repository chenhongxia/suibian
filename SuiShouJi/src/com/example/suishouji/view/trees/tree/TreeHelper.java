...
public class TreeHelper {
	...
	public static void setNodeIcon(Node node) {
		if (node.getChildrenNodes().size() > 0 && node.isExpand()) {
			node.setIcon(R.drawable.arrowed_down);
		} else if (node.getChildrenNodes().size() > 0 && !node.isExpand()) {
			node.setIcon(R.drawable.arrowed_right);
		} else
			node.setIcon(-1);
	}

	public static void setNodeChecked(Node node, boolean isChecked,boolean mSingleChecked) {
		// 自己设置是否选择
		...
		/** 父节点处理 */
		setParentNodeChecked(node,mSingleChecked);
	 ...
	 * @param node
	 */
	public  static void setParentNodeChecked(Node node,boolean mSingleChecked) {

		if (mSingleChecked){
			if (!node.isRoot()) {
				Node rootNode = node.getParent();
				rootNode.setChecked(true);
				setParentNodeChecked(rootNode, true);
			}
		}else{
			/** 非根节点 */
			if (!node.isRoot()) {
				...
				setParentNodeChecked(rootNode, false);
			}
		}
